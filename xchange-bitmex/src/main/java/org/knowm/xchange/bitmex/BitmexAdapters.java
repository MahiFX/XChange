...
public class BitmexAdapters {
    ...
    public static OpenPosition adaptPosition(BitmexPosition bitmexPosition) {
        Instrument instrument = adaptSymbolToCurrencyPair(bitmexPosition.getSymbol());
        OpenPosition.Type type = bitmexPosition.getCurrentQty().signum() > 0 ? OpenPosition.Type.LONG : OpenPosition.Type.SHORT;
        return new OpenPosition(instrument,
                type,
                bitmexPosition.getCurrentQty().abs(),
                bitmexPosition.getAvgEntryPrice(),
                bitmexPosition.getLiquidationPrice(),
                bitmexPosition.getUnrealisedPnl()
        );
    }
